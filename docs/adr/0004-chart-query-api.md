...
调用面是一份固定的 `Ziwei` 本命盘，加上轻量 `ZiweiView`（本命 / 第几步大限 / 某农历年流年）。视图只改宫职→地支的贴标，以及大限/流年四化叠加；不复制整盘，不重算飞边、星位、生年四化、来因宫。`Palace::role()` 始终是本命宫职；大限/流年宫职只通过带 `view` 的查询得到。结果类型字段私有（ADR-0010）。v1 **不**提供 `with_view` / `ZiweiHandle`：视图相关方法一律在 `Ziwei` 上显式传 `ZiweiView`，避免半成品句柄与第二门面。
