...
引擎产出的多字段结果类型不得被外部自由构造。`Palace`、`ZiweiFly`、`DecadeStep`、`LayerTransformation`、`DecadeYear` 的字段私有，只经 crate 内装配；对外只提供 `const` 只读 getter。与输入侧 `ZiweiBirth` / `ZiweiInput` / `DecadeIndex` 同一模式：不变量由类型系统保证，不靠调用方自觉。
...
| `ZiweiFly` | 源支宫干 × 四化 → 星 → 本命落宫为目标支 |
| `DecadeStep` | `age_end = age_start + 9`；`stem` 为大限命支上的本命宫干 |
| `LayerTransformation` | 干 × 四化 → 星 → 本命落宫 |
...
step.step() / step.ming_branch() / step.age_start() / step.age_end() / step.stem()
xf.transformation() / xf.star() / xf.branch()
