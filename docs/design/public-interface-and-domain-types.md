...
```rust
let natal = create_from_birth(ZiweiBirth::try_new(/* ... */)?);
let natal = create_from_input(ZiweiInput::try_new(/* ... */)?);
```
...
Star.name() / category() / galaxy() / origin_transformation() / self_transformations()
...
`Gender`、`Branch`、`Stem`、`PalaceName`、`StarName`、`StarCategory`、`StarGalaxy`、`Transformation`、`Zodiac`、`DecadeDirection`、`DecadeIndex` 是稳定领域身份或值。

`Gender` 固定使用 `Yang / Yin` 表示阳男/阴女；阴阳是 core 的领域值，`Male / Female` 不属于公开内核语言。

宫名、星名、斗系及四化的简繁体文案不属于 core，由 `ziwei` 外层提供。
